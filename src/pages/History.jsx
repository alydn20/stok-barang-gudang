...
import {
  ...
  ClipboardList, SearchX, RefreshCw,
} from 'lucide-react'
    ...
    <div className="page">
      ...
      <div style={s.filterBox}>
        <div style={s.presetRow}>
          ...
          <button onClick={() => { const end = new Date(); const start = new Date(); start.setDate(end.getDate() - (preset === 0 ? 6 : preset - 1)); load(toDateStr(start), toDateStr(end)) }}
      ...
      {loading ? (
        ...
        <div style={s.empty}>
          <SearchX size={32} color="#CBD5E1" />
          <p style={{ fontSize: 14, color: '#94A3B8', marginTop: 8 }}>Tidak ada transaksi</p>
        </div>
      ) : (
        <div style={s.list}>
          {filtered.map((r, i) => (
            <div key={i} style={s.row}>
              <div style={{ ...s.typeIcon, background: r.tipe === 'MASUK' ? '#F0FDF4' : '#FEF2F2' }}>
                {r.tipe === 'MASUK'
                  ...
                  : <ArrowUpCircle   size={18} color="#DC2626" />
                }
              </div>
              <div style={s.rowBody}>
                <div style={s.rowTop}>
...
const s = {
  ...
  presetBtn:   { padding: '5px 13px', borderRadius: 99, border: '1.5px solid #E2E8F0', background: '#fff', fontSize: 12, fontWeight: 600, cursor: 'pointer', color: '#64748B', transition: 'all 0.15s' },
  ...
  typeBtn:     { padding: '5px 12px', borderRadius: 99, border: '1.5px solid #E2E8F0', background: '#fff', fontSize: 11, fontWeight: 600, cursor: 'pointer', color: '#64748B', transition: 'all 0.15s' },
